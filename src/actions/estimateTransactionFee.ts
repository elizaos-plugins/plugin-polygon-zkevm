...
export const estimateTransactionFeeAction: Action = {
  ...
  description:
    "Estimate gas limit and total fee for a transaction on Polygon zkEVM",
  ...
  validate: async (
    ...
    const hasFeeKeywords = feeEstimationKeywords.some((keyword) =>
    ...
    const hasValue =
      /\d+(?:\.\d+)?\s*eth/i.test(messageText) || messageText.includes("send");

    logger.debug(
      "[estimateTransactionFeeAction] Validation check:",
      String({
        hasFeeKeywords,
        hasAddress,
        hasValue,
  ...
  handler: async (
    ...
    try {
      ...
      if (!alchemyApiKey && !zkevmRpcUrl) {
        ...
        logger.error(
          `[estimateTransactionFeeAction] Configuration error: ${errorMessage}`,
        );
        if (callback) {
          await callback({
            text: errorMessage,
      ...
      try {
        transactionInput = await callLLMWithTimeout<{
          ...
          runtime,
          state,
          estimateTransactionFeeTemplate,
        ...
        if (transactionInput?.rawTransaction && !transactionInput?.to) {
          logger.info(
            "[estimateTransactionFeeAction] Processing raw transaction...",
          );
      ...
      if (alchemyApiKey) {
        ...
        logger.info(
          "[estimateTransactionFeeAction] Using RPC provider:",
          zkevmRpcUrl,
        );
      }
      ...
      logger.info(
        "[estimateTransactionFeeAction] Building transaction object...",
      );
      ...
      if (transactionInput.value) {
        ...
        transaction.value =
          "0x" + BigInt(Math.floor(ethValue * 1e18)).toString(16);
      }
      ...
      logger.info(
        "[estimateTransactionFeeAction] Gas estimation completed:",
        gasLimit.toString(),
      );
      ...
      if (transactionInput.priorityFee) {
        ...
        logger.info(
          "[estimateTransactionFeeAction] Getting current gas price...",
        );
        const gasPriceHex = await provider.send("eth_gasPrice", []);
        gasPrice = BigInt(gasPriceHex);
        logger.info(
          "[estimateTransactionFeeAction] Gas price retrieved:",
          gasPrice.toString(),
        );
      }
...
${transactionInput.priorityFee ? `\n**Note:** Used custom priority fee of ${transactionInput.priorityFee} Gwei` : ""}`;
      ...
      if (callback) {
        await callback({
          text: responseText,
          content: { success: true, ...responseData },
        });
      }
