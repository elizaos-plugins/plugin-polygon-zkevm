...
export const getTransactionReceiptAction: Action = {
  ...
  description:
    "Gets the transaction receipt for a given hash on Polygon zkEVM.",
  ...
  handler: async (
    ...
    try {
      ...
      if (!txHashMatch) {
        const errorText =
          "Please provide a valid transaction hash (0x... 64 characters) to get the transaction receipt.";
        await callback({
          text: errorText,
          content: { success: false, error: errorText },
        });
        return {
          success: false,
          text: `❌ ${errorText}`,
          ...
          data: {
            actionName: "POLYGON_ZKEVM_GET_TRANSACTION_RECEIPT",
            error: errorText,
          },
...
🔢 Transaction Index: ${receipt.index}`;
      ...
      await callback({
        text: responseText,
        content: { success: true, hash: receipt.hash },
      });
      return {
        success: true,
        text: responseText,
