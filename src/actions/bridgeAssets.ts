  ...
  handler: async (
    ...
    if (!privateKey) {
      ...
      return {
        success: false,
        text: `❌ ${errorMessage}`,
        values: { bridgeSucceeded: false, error: true, errorMessage },
        data: {
          actionName: "POLYGON_ZKEVM_BRIDGE_ASSETS",
          error: errorMessage,
        },
    ...
    try {
      ...
      if (bridgeParams?.error) {
        logger.error(
          "[bridgeAssetsAction] LLM returned an error:",
          bridgeParams?.error,
        );
      ...
      if (!bridgeParams?.amount || !bridgeParams?.direction) {
        throw new Error(
          "Missing required bridge parameters: amount and direction are required.",
        );
      }
      ...
      const errorMessage = `[bridgeAssetsAction] Failed to extract bridge parameters from input: ${error instanceof Error ? error.message : String(error)}`;
      if (callback) {
        await callback({
          text: errorMessage,
          content: { success: false, error: errorMessage },
    ...
    try {
      ...
      const bridgeAddress =
        bridgeParams.direction === "deposit"
          ? BRIDGE_ADDRESSES.mainnet
          : BRIDGE_ADDRESSES.zkevm;
      ...
      if (
        ...
        if (bridgeParams.maxFeePerGas) {
          txOptions.maxFeePerGas = parseUnits(
            bridgeParams.maxFeePerGas,
            "gwei",
          );
        }
        if (bridgeParams.maxPriorityFeePerGas) {
          txOptions.maxPriorityFeePerGas = parseUnits(
            bridgeParams.maxPriorityFeePerGas,
            "gwei",
          );
        }
        ...
        txHash = tx.hash;
        logger.info(
          `[bridgeAssetsAction] ETH bridge transaction sent: ${txHash}`,
        );
      } else {
        // Bridge ERC20 token
        ...
        if (allowance < amount) {
          ...
          logger.info(
            `[bridgeAssetsAction] Token approval confirmed: ${approveTx.hash}`,
          );
        }
        ...
        txHash = tx.hash;
        logger.info(
          `[bridgeAssetsAction] ERC20 bridge transaction sent: ${txHash}`,
        );
      }
      ...
      for (const log of receipt.logs) {
        try {
          ...
          if (parsedLog && parsedLog.name === "BridgeEvent") {
            ...
            logger.info(
              `[bridgeAssetsAction] Bridge ticket ID: ${bridgeTicketId}`,
            );
            break;
          }
        } catch (error) {
      ...
      const successText = `✅ Assets bridged successfully!
...
Please wait for the transaction to be confirmed.`;
      ...
      if (callback) {
        await callback({
          text: successText,
          content: { success: true, transactionHash: txHash },
        });
      }
      ...
      return {
        success: true,
        text: successText,
        values: { bridgeSucceeded: true, transactionHash: txHash },
        data: {
          actionName: "POLYGON_ZKEVM_BRIDGE_ASSETS",
          transactionHash: txHash,
          amount: bridgeParams.amount,
          sourceNetwork:
            sourceNetworkId === NETWORK_IDS.ethereum ? "Ethereum" : "zkEVM",
          destinationNetwork:
            destinationNetworkId === NETWORK_IDS.ethereum
              ? "Ethereum"
              : "zkEVM",
          bridgeTicketId,
          depositCount,
